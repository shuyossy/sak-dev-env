...
| 2026-04-25 | `rules/spotbugs/exclude.xml` | `EI_EXPOSE_REP` / `EI_EXPOSE_REP2` を `~.*\.(domain\|entity\|dto)\..*` 配下のクラスに限定して除外 | JPA エンティティは ORM のため、record ベースの DTO は言語仕様上 field 参照を露出する必要があり構造的に避けられない                                                                                              |
